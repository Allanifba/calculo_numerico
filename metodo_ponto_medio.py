...
def fmetodo_ponto_medio():
    while True:
        print('-----------------------------------  DESCRIÇÃO  ------------------------------------')
        print( 'Programa que aplica o Método do Ponto Médio para aproximar a solução de um PVI no\n '
              'intervalo [a,b].' )
        print('-----------------------------------  INSTRUÇÕES  -----------------------------------')
        ...
        while i<=n:
            omega = omega + h*f(t + h/2,omega + (h/2)*f(t,omega))
            t = a + i*h
            i = i + 1
            print('i=%d, omega = %0.9f' % (i-1, omega))
